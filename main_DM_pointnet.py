...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='ModelNet', help='dataset')
    ...
    for exp in range(args.num_exp):
        ...
        log_file = get_time()+'eval_record_lr'+str(args.lr_img)+'_ratio_'+str(args.ratio)+'_num_point_'+str(args.num_point)+'_dataset_'+str(args.dataset)+'_model_'+str(args.model)+'_init_'+str(args.init)+'.txt'
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                ...
                # np.save('syn_data_trans/image_syn_it'+str(it)+args.dataset,np.array(image_syn_eval.cpu()))
                # np.save('syn_data_trans/label_syn_it'+str(it)+args.dataset,np.array(label_syn_eval.cpu()))
                ''' visualize and save '''
            ...
            real_centers = []
            real_centers = torch.FloatTensor(real_centers).to(args.device)
            ...
            if 'BN' not in args.model: # for ConvNet
                ...
                for c in range(num_classes):
                    ...
                    #real_centers = torch.cat([real_centers, torch.mean(output_real, dim=0).unsqueeze(0)], dim=0)
                    #print(real_centers.shape)
