...
def main():
    ...
    parser.add_argument('--saved_data_path', type=str, default='syn_data_trans/image_syn_it1000ModelNet40.npy', help='data path for saved condensed data')
    parser.add_argument('--saved_label_path', type=str, default='syn_data_trans/label_syn_it1000ModelNet40.npy', help='data path for saved condensed data label')
    ...
    print('images_all', images_all.shape)
    print('labels_all', labels_all.shape)


    condensed_images_all = np.load(args.saved_data_path)
    condensed_labels_all = np.squeeze(np.load(args.saved_label_path))
    ...
    condensed_labels_all = torch.tensor(condensed_indices_class, dtype=torch.long, device=args.device)
    ...
    for seed_cl in range(1):
        ...
        elif args.method == 'pcc':
            images_train_all = []
            labels_train_all = []
            for step in range(args.steps):
                classes_current = class_order[step * num_classes_step: (step + 1) * num_classes_step]
                images_train_all += [torch.cat([get_condensed_images(c, args.ipc) for c in classes_current], dim=0)]
                labels_train_all += [torch.tensor([c for c in classes_current for i in range(args.ipc)], dtype=torch.long, device=args.device)]
        else:
